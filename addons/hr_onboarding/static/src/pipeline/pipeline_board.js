...
const FIELDS = [
    "display_name", "first_name", "job_id", "department_id", "manager_id", "hr_user_id",
    "joining_date", "progress", "countdown", "is_delayed", "priority", "stage_id", "kanban_state",
];

export class HrOnboardingPipeline extends Component {
    static template = "hr_onboarding.PipelineBoard";
    static props = ["*"];
    ...
    async loadData() {
        ...
        const records = await this.orm.searchRead("hr.onboarding", [], FIELDS);
    ...
    async moveRecord(recordId, sourceStageId, targetStageId) {
        ...
        await this.orm.write("hr.onboarding", [recordId], { stage_id: targetStageId });
    }
    ...
    openJourney(recordId) {
        this.action.doAction({
            ...
            params: { onboarding_id: recordId },
        });
    }

    async sendDocumentRequest(recordId) {
        await this.orm.call("hr.onboarding", "action_send_document_request", [[recordId]]);
...
registry.category("actions").add("hr_onboarding_pipeline", HrOnboardingPipeline);
