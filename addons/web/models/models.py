...
class Base(models.AbstractModel):
    ...
    def web_read_group(
        ...
        *,
        ...
        groupby = self._normalize_groupby_specs(groupby)
        if not groupby:
            raise ValueError("Invalid groupby list passed to web_read_group: no valid groupby specification")
    ...
    def _normalize_groupby_specs(self, groupby: Sequence[str]) -> list[str]:
        return [spec.strip() for spec in groupby if isinstance(spec, str) and spec.strip()]
    ...
    def formatted_read_grouping_sets(
        ...
        *,
        ...
        :raise AccessError: if user is not allowed to access requested
            information
        """
        grouping_sets = [tuple(self._normalize_groupby_specs(groupby)) for groupby in grouping_sets]
    ...
    def formatted_read_group(
        ...
        :raise AccessError: if user is not allowed to access requested
            information
        """
        groupby = tuple(self._normalize_groupby_specs(groupby))
        aggregates = tuple(agg.replace(':recordset', ':array_agg') for agg in aggregates)
