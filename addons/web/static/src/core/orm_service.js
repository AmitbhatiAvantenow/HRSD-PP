...
function normalizeGroupBy(groupby) {
    return groupby.map((g) => (typeof g === "string" ? g.trim() : g)).filter((g) => g);
}
...
export class ORM {
    ...
    formattedReadGroupingSets(model, domain, grouping_sets, aggregates, kwargs = {}) {
        ...
        validatePrimitiveList("aggregates", "string", aggregates);
        const normalizedGroupingSets = grouping_sets.map((groupby) => normalizeGroupBy(groupby));
        return this.call(model, "formatted_read_grouping_sets", [], {
            domain,
            grouping_sets: normalizedGroupingSets,
            aggregates,
            ...kwargs,
        }).then((res) => {
